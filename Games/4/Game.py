...
pygame.init() # init
gamedisplay = pygame.display.set_mode((360,360)) # width and height
...
#musics
pygame.mixer.music.load("../../Sounds/jungle_2.wav")
...
def check():
    ...
    if player_ar == "[0][2]":
        ...
        if level[0][0] == 9:
            ...
            coin = cur.fetchall()
            cur.execute("UPDATE info_cc SET Coin=" + str(coin[0][0] + 5) + " WHERE rowid=1;")
            cur.execute("UPDATE info_play SET level = 10 WHERE rowid=1;")
...
def output(out):
    ...
    #if
    if player_ar == "[4][0]":
        answer = ((in_1 * 10) + 7) * 5 - (2 * in_2) + in_3 - 5
